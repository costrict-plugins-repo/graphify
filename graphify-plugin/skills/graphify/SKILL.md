...
### Step 1 - Ensure graphify CLI is installed

This step runs a single pre-flight block that checks Python, installs graphify if needed, and saves the interpreter path. Do not split it — run the entire block as one bash invocation. Replace `INPUT_PATH` with the actual path.

**Phase A — Find Python and check version ≥ 3.10:**
...
if [ -z "$PYTHON" ]; then
    ...
    if command -v python3 >/dev/null 2>&1 || command -v python >/dev/null 2>&1; then
        _FOUND_VER=$(python3 --version 2>/dev/null || python --version 2>/dev/null || echo "unknown")
        echo "ERROR: Python 3.10+ is required. Found: $_FOUND_VER"
        echo "Upgrade Python to 3.10 or newer:"
...
echo "[graphify] Python $("$PYTHON" --version) found"
```

If Phase A exits with error, stop and tell the user what to install. Do not proceed.

**Phase B — Install graphify if missing:**

```bash
if "$PYTHON" -c "import graphify" 2>/dev/null; then
    echo "[graphify] CLI already installed ($("$PYTHON" -m graphify --version 2>/dev/null || echo ok))"
else
    echo "[graphify] CLI not found. Installing graphifyy..."
    ...
    if ! "$PYTHON" -c "import graphify" 2>/dev/null; then
        echo "ERROR: Failed to install graphify. Try manually:"
...
If Phase B exits with error, stop and tell the user the manual install commands. Do not proceed.

**Phase C — Resolve the graphify binary and save interpreter path:**
...
echo "[graphify] Ready — Python at $(cat graphify-out/.graphify_python)"
```

After all three phases succeed, move straight to Step 2. Do not re-run Phase A or B in later steps.
